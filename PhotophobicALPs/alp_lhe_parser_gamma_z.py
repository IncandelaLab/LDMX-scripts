...
def find_decay_vertex(three_momentum, beam_vertex, mass):
    c = 299_792_458 * 1000 # [c] = mm / s
    hbar = 6.582119569 * 10**(-16) * 10**(-9) # [hbar] = GeV s

    decay_length = gamma.rvs(2, loc = 350, scale = 15)
    ...
    gamma_fac = energy / mass
    velo_vec = three_momentum  * c / (gamma_fac * mass)
...
def main():
    ...
    ALP_file = args.output_dir + f'/AlpToe+e-_{str(args.mass)}GeV_seed{str(args.seed)}_gamma_prod.lhe'
    decay_file = args.output_dir + f'/AlpToe+e-_{str(args.mass)}GeV_seed{str(args.seed)}_gamma_decay.lhe'
    ...
    with open(input_file, 'r') as input_lhe, open(ALP_file, 'w') as ALP_lhe, open(decay_file, 'w') as decay_lhe:
        ...
        for event in events:
            ...
            decay_vertex = find_decay_vertex(ALP_three_momentum, np.array([beam_x, beam_y, beam_z]), args.mass)
