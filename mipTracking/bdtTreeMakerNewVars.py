...
def isolatedhit(mcid,layer,hitlist):
  for hit in hitlist:
    hitlayer = hit[2]
    hitmcid = hit[4]
    if hitlayer == layer:
      for id in neighborList[mcid]:
        if hitmcid == id and not mcid == id:
 #          print mcid,hitmcid,neighborList[mcid]
 #         print "not isolated"
           return False
  return True

def isolatedend(mcid,layer,hitlist):
  for hit in hitlist:
    hitlayer = hit[2]
    hitmcid = hit[4]
    if hitlayer == layer:
...
def checkforhits(mcid,layer,hitlist,track, isolation):
  for hit in hitlist:
    hitlayer = hit[2]
    hitmcid = hit[4]
    if hitlayer == layer-1:
      for id in neighborList[mcid]:
        if hitmcid == id:
          if isolation:
            if not isolatedhit(hitmcid,hitlayer,hitlist):
              return track
          track.append(10000*hitlayer+hitmcid)
          return checkforhits(hitmcid,hitlayer,hitlist,track,isolation)
  return track

def gettrackpositions(track):
  ret = [[-999,-999]]
  if len(track)<2:
    return ret
  mcid0 = track[0]%10000
  pos0 = [-999,-999,-999]
  pos0[0] = cellMap[mcid.index(mcid0)][1]
  pos0[1] = cellMap[mcid.index(mcid0)][2]
  pos0[2] = layerZs[(track[0]-track[0]%10000)/10000]

  mcid1 = track[1]%10000
  pos1 = [-999,-999,-999]
  pos1[0] = cellMap[mcid.index(mcid1)][1]
  pos1[1] = cellMap[mcid.index(mcid1)][2]
  pos1[2] = layerZs[(track[len(track)-1]-track[len(track)-1]%10000)/10000]

  pvec = [-999,-999,-999]  
  pvec[0] = (pos1[0]-pos0[0])/math.sqrt((pos0[0]-pos1[0])**2+(pos0[1]-pos1[1])**2+(pos0[2]-pos1[2])**2)
  pvec[1] = (pos1[1]-pos0[1])/math.sqrt((pos0[0]-pos1[0])**2+(pos0[1]-pos1[1])**2+(pos0[2]-pos1[2])**2)
  pvec[2] = (pos1[2]-pos0[2])/math.sqrt((pos0[0]-pos1[0])**2+(pos0[1]-pos1[1])**2+(pos0[2]-pos1[2])**2)

  ret = getElectronTrajectory(pvec,pos0)
  #if not (pvec[0]==0 and pvec[1]==0):
  #print "pos0 is  ", pos0
  #print "pos1 is  ", pos1
  #  print "pvec is  ", pvec

  #  print "positions is   ", ret
  return ret
...
class bdtTreeMaker:
    ...
    def run(self):
        while self.event_count < self.intree.GetEntries():   # and self.event_count<5000:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--swdir', dest='swdir',  default='/nfs/slac/g/ldmx/users/jbargem/ldmx-sw/ldmx-sw-install', help='ldmx-sw build directory')
