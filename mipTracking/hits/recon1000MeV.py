...
adds = "recon_1000MeV_0"
savePath = "/nfs/slac/g/ldmx/users/jmlazaro/ldmx-sw/scripts/mipTracking/outHitTxts/"
...
def getElectronTrajectory(eventcount,pvec,pos,grouptag):
    ...
    for layerZ in layerZs:
        ...
        hitInfo = [eventcount,posX,posY,layerZ,0,0,grouptag]
...
class bdtTreeMaker:
	...
	self.event_count += 1
		...
		hitInfo = [self.event_count,hitX,hitY,layerZ,hitE,mcid_val,0]
		with open(savePath + "hits_" + adds + ".txt",'a') as hits:
		    writer = csv.writer(hits)
	    ...
	    if distanceEle < radius_68[ir][layer] and distancePhoton > radius_68[ip][layer]:
                ...
                outside68WgtCentroidCoords = (outside68WgtCentroidCoords[0] + hitX*hitE, outside68WgtCentroidCoords[1] + hitY*hitE)
                outside68HitPositions.append((hitX,hitY,layer,hitE,mcid_val))
		hitInfo = [self.event_count,hitX,hitY,layerZ,hitE,mcid_val,1]
		with open(savePath + "hits_" + adds + ".txt",'a') as hits:
		    writer = csv.writer(hits)
		    for entery in hitInfo:    
			hits.write(str(entery) + " ")
	    ...
	    if distanceEle > radius_68[ir][layer] and distancePhoton < radius_68[ip][layer]:
                ...
                outside68WgtCentroidCoords = (outside68WgtCentroidCoords[0] + hitX*hitE, outside68WgtCentroidCoords[1] + hitY*hitE)
                outside68HitPositions.append((hitX,hitY,layer,hitE,mcid_val))
		hitInfo = [self.event_count,hitX,hitY,layerZ,hitE,mcid_val,2]
		with open(savePath + "hits_" + adds + ".txt", 'a') as hits:
		    writer = csv.writer(hits)
		    for entery in hitInfo:    
			hits.write(str(entery) + " ")
	    ...
	    if distanceEle < radius_68[ir][layer] and distancePhoton < radius_68[ip][layer]:
                ...
                outside68WgtCentroidCoords = (outside68WgtCentroidCoords[0] + hitX*hitE, outside68WgtCentroidCoords[1] + hitY*hitE)
                outside68HitPositions.append((hitX,hitY,layer,hitE,mcid_val))
		hitInfo = [self.event_count,hitX,hitY,layerZ,hitE,mcid_val,3]
		with open(savePath + "hits_" + adds + ".txt",'a') as hits:
		    writer = csv.writer(hits)
		    for entery in hitInfo:    
			hits.write(str(entery) + " ")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--swdir', dest='swdir',  default='/nfs/slac/g/ldmx/users/jmlazaro/ldmx-sw/ldmx-sw-install', help='ldmx-sw build directory')
