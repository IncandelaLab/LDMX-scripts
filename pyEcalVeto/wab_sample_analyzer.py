...
branches_info['targetSPHits']= { #Total number of hits on the target scoring plane z1 (The plane immediately downstream of the target)
    'rtype': float,
    'default':0.5
    }
branches_info['targetSPEnergy']= { #Total energy on the target scoring plane z1 (The plane immediately downstream of the target)
    'rtype': float,
    'default':0.5
    }
...
branches_info['gPx']= { #The XComponent of the main photon Momentum
    'rtype': float,
    'default':0.5
    }
branches_info['gPy']= { #The YComponent of the main photon Momentum
    'rtype': float,
    'default':0.5
    }
branches_info['gPz']= { #The ZComponent of the main photon Momentum
    'rtype': float,
    'default':0.5
    }
branches_info['gx']= {  #The XComponent of the main photon hit on the target SP
    'rtype': float,
    'default':0.5
    }
branches_info['gy']= {  #The XComponent of the main photon hit on the target SP
    'rtype': float,
    'default':0.5
    }
branches_info['gz']= {  #The XComponent of the main photon hit on the target SP
    'rtype': float,
    'default':0.5
    }
branches_info['gPMagnitude']= {  #The magnitude of the main photon Momentum
    'rtype': float,
    'default':0.5
    }
branches_info['gThetaNew']= { #The angle of the photon with respect to the z-direction (Truth info)
    'rtype': float,
    'default':0.5
    }
branches_info['egThetaNew']= { #The angle between the electron and the photon (Truth)
    'rtype': float,
    'default': 0.
    }
...
branches_info['ePy']= {     #The YComponent of the recoil electron Momentum
    'rtype': float,
    'default': 0.
    }
branches_info['ePx']= {     #The XComponent of the recoil electron Momentum
    'rtype': float,
    'default': 0.
    }
...
def main():
    ...
    for proc in procs:
        ...
        '''# Branches needed
        proc.eventHeader  = proc.addBranch('EventHeader', 'EventHeader')
        proc.ecalVeto = proc.addBranch( 'EcalVetoResult', 'EcalVeto_v13' )
        proc.hcalVeto = proc.addBranch( 'HcalVetoResult', 'HcalVeto_v13' )
        proc.targetSPHits = proc.addBranch('SimTrackerHit', 'TargetScoringPlaneHits_v13')
        proc.ecalSPHits   = proc.addBranch('SimTrackerHit', 'EcalScoringPlaneHits_v13')
        proc.ecalRecHits = proc.addBranch( 'EcalHit', 'EcalRecHits_v13' )
        proc.hcalRecHits = proc.addBranch( 'HcalHit', 'HcalRecHits_v13' )
        proc.simParticles = proc.addBranch('SimParticle', 'SimParticles_v13')
        proc.hcalWABVeto = proc.addBranch('HcalVetoResult', 'HcalWABVetoes_v13')
        proc.ecalSimHits  = proc.addBranch('SimCalorimeterHit', 'EcalSimHits_v13')
        proc.RecoilSimHits = proc.addBranch('SimTrackerHit', 'RecoilSimHits_v13')
...
def event_process(self):
    ...
    feats['hcalWABScore'] = self.hcalWABVeto.passesVeto()
...
#    feats['nSimHits'    ] = len([simHit for simHit in self.ecalSimHits])
    ...
    ############################################
    #Recoil Tracker simulated imformation
    #Each entry of each list is a vector that stores the corresponding imformation of a simulated hit 
    #For example: the ith entry of 'recoilpositionlist' stores the position vector of the ith hit simulated
    ############################################
    ...
    for hit in self.RecoilSimHits:
        recoilpositionlist += [hit.getPosition()]
        recoilmomentumlist += [hit.getMomentum()]
        recoilIDlist += [hit.getPdgID()]
    feats['recoilPositionList'] = recoilpositionlist
    feats['recoilMomentumList'] = recoilmomentumlist
    feats['recoilHitIDList'] = recoilIDlist
    '''
    rl1Ne = 0
    rtotalNe= 0
    rl1E = 0
